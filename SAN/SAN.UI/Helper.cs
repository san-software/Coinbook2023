...
namespace SAN.UI.Controls
{
	...
	public class Helper
	{
		...
		public static bool IsValidNumber(string text, int nachkommaStellen)
		{
			Single num;
			bool result = true;
			int nk = 0;
		...
		public static bool CheckInput(string text, TextBoxTyp typ, int nachkommaStellen)
		{
			bool result = true;
			char[] array;
			int slash = 0;
		...
		//Sperrt alle controls mit sub-controls
		public static void LockAllControls(Control.ControlCollection controls)
		{
			foreach (Control c in controls)
			{
				...
				{
					case "TextBoxEx":
						((TextBoxEx)c).ReadOnly = true;
						break;
					case "ComboBoxEx":
						((ComboBoxEx)c).ReadOnly = true;
						break;
					case "DateTimePickerEx":
						((DateTimePickerEx)c).ReadOnly = true;
						break;
					default:
						LockAllControls(c.Controls);
						break;
				}
			}
		}

		//Prüfen ob die angegebene Form schon geladen ist
		...
		{
			if (c.GetType().BaseType.Name != "FormEx" && c.GetType().BaseType.Name != "Form")
			{
				switch (c.GetType().Name)
				{
					case "GroupBoxEx":
						((GroupBoxEx)c).Enabled = false;
						break;

					case "PanelEx":
						((PanelEx)c).Enabled = false;
						break;
		...
		{
			if (c.GetType().BaseType.Name != "FormEx")
			{
				switch (c.GetType().Name)
				{
					case "GroupBoxEx":
						((GroupBoxEx)c).Enabled = true;
						break;

					case "PanelEx":
						((PanelEx)c).Enabled = true;
						break;
