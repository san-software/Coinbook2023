...
namespace SAN.UI.DataGridView
{
	...
	public class GridFilterCollection : ReadOnlyCollectionBase
	{
		...
		private Hashtable columnsToGridFiltersHash;
		...
		{
            columnsToGridFiltersHash = (Hashtable)columnsToGridFiltersHash.Clone();

            foreach (DataGridViewColumn column in columns)
			{
				IGridFilter gridFilter = (IGridFilter)columnsToGridFiltersHash[column];
				if (gridFilter != null)
		...
		public IGridFilter this[DataGridViewColumn column]
		{
			get 
			{
                if (base.InnerList.Contains(columnsToGridFiltersHash[column]))
                    return (IGridFilter)columnsToGridFiltersHash[column];
				else
					return null;
			}
		}
		...
		{
            foreach (DataGridViewColumn column in columnsToGridFiltersHash.Keys)
                if (column.Name == name)
                    return this[column];
			return null;
		}
		...
		public GridFilterCollection FilterByGridFilterType(Type dataType, bool exactMatch)
		{
			...
			return new GridFilterCollection(filtered, columnsToGridFiltersHash);
		}
