...
namespace SAN.UI.DataGridView
{
    ...
    public class BindingListView<T> : IBindingListView, ITypedList
    {
        ...
        private string currentFilterExpression = string.Empty;
        ...
        {
            properties = ListBindingHelper.GetListItemProperties(typeof(T));
            filterTable = new DataTable("FilterTable");
            foreach (PropertyDescriptor property in properties)
                filterTable.Columns.Add(property.Name, property.PropertyType);
        }
        ...
        public void ApplySort(ListSortDescriptionCollection sorts)
        {
            ...
            sortIndices = new int[innerList.Count];
            object[] items = new object[innerList.Count];
            for (int i = 0; i < sortIndices.Length; i++)
            {
                sortIndices[i] = i;
                items[i] = innerList[i];
            }
            Array.Sort(items, sortIndices, new GenericComparer(sorts));
            this.Filter = currentFilterExpression;
        }
        ...
        {
            get { return currentFilterExpression; }
            set
            {
                filterIndices = null;
                currentFilterExpression = string.Empty;

                if (value.Length > 0)
                {
                    DataFilter dataFilter = new DataFilter(value, filterTable);
                    List<int> filteredIndices = new List<int>();

                    int count = this.Count;
                    int propertiesCount = properties.Count;
                    DataRow row = filterTable.NewRow();
                    ...
                    filterIndices = filteredIndices.ToArray();
                    currentFilterExpression = value;
                }
        ...
        public object this[int index]
        {
            get
            {
                if (filterIndices != null)
                    index = filterIndices[index];

                if (sortIndices != null && index < sortIndices.Length)
        ...
        public int Count
        {
            get { return filterIndices == null ? innerList.Count : filterIndices.Length; }
        }
