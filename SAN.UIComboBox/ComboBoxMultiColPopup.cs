...
{
	public delegate void AfterRowSelectEventHandler(object sender, string text, int selectedItem);
	/// <summary>
	/// Summary description for ComboBoxMultiColPopup.
	/// </summary>
	public class ComboBoxMultiColPopup : System.Windows.Forms.Form
	{
		/// <summary>
		...
		public ComboBoxMultiColPopup(DataRow[] drows) 
		{
			InitializeComponent();
		...
		public ComboBoxMultiColPopup(DataTable dtable, ref DataRow selRow, string colsToDisplay) 
		{
			InitializeComponent();
			this.dataTable = dtable;
			this.selectedRow = selRow;
			this.columnsToDisplay = colsToDisplay.Split(",".ToCharArray());
		...
		private void InitializeComponent()
		{
			...
			this.Name = "ComboBoxMultiColPopup";
			this.ShowInTaskbar = false;
			this.StartPosition = System.Windows.Forms.FormStartPosition.Manual;
			this.KeyDown += new System.Windows.Forms.KeyEventHandler(this.ComboBoxMultiColPopup_KeyDown);
			this.Leave += new System.EventHandler(this.ComboBoxMultiColPopup_Leave);
			this.Deactivate += new System.EventHandler(this.ComboBoxMultiColPopup_Deactivate);
		...
		private void InitializeGridProperties() {
			...
			lstvMyView.HeaderStyle = headerStyle;
			lstvMyView.GridLines = GridLinesMultiColumn;
		...
		private void ComboBoxMultiColPopup_Deactivate(object sender, System.EventArgs e) {
			this.Close();
		}

		private void ComboBoxMultiColPopup_Leave(object sender, System.EventArgs e) {
			this.Close();
		}

		private void ComboBoxMultiColPopup_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e) {
			...
			if(e.KeyData==Keys.Enter){
				if(lstvMyView.SelectedItems.Count>0){
					...
					if(AfterRowSelectEvent!=null)
						AfterRowSelectEvent(this,selectedRow[DisplayMember].ToString(),lstvMyView.SelectedIndices[0]);
				}
				this.Close();	
			}
		}
		...
		{
			if (lstvMyView.Items.Count > 0)
				this.Height = maxDropDownItems * lstvMyView.Items[0].Bounds.Height + 5;
		}
