...
namespace Coinbook.Helper
{
    public static class CoinbookHelper
    {
        ...
        public static string PluginPath { get { return Path.Combine(Application.StartupPath, "Plugins"); } }
        ...
        public static string ModulKey {get;set;}
        ...
        public static Color ColorHeader4
        {
            get
            {
                return Color.Gold;
        ...
        public static BindingList<Sammlung> LoadSammlungsliste(string guid, int index, enmDoublette typ = enmDoublette.Alle)
        {
        ...
        {
            MessageBoxAdv.Show("Coinbook wird neu gestartet", Application.ProductName);
        ...
        public static int StartProgram(string program, string param, string path = null)
        {
            if (path == null)
                program = Path.Combine(Application.StartupPath, program);
        ...
        public static void CalculateOwnPrices(int index, PreisEventArgs args)
        {
            if (args.SPreis != 0)
            {
                MuenzkatalogFiltered[index].SPreis = args.SPreis;
                MuenzkatalogFiltered[index].Farbe = "1";
            }

            if (args.SPPreis != 0)
            {
                MuenzkatalogFiltered[index].SPPreis = args.SPPreis;
                MuenzkatalogFiltered[index].Farbe = "1";
            }

            if (args.SSPreis != 0)
            {
                MuenzkatalogFiltered[index].SSPreis = args.SSPreis;
                MuenzkatalogFiltered[index].Farbe = "1";
            }

            if (args.SSPPreis != 0)
            {
                MuenzkatalogFiltered[index].SSPPreis = args.SSPPreis;
                MuenzkatalogFiltered[index].Farbe = "1";
            }

            if (args.VZPreis != 0)
            {
                MuenzkatalogFiltered[index].VZPreis = args.VZPreis;
                MuenzkatalogFiltered[index].Farbe = "1";
            }

            if (args.VZPPreis != 0)
            {
                MuenzkatalogFiltered[index].VZPPreis = args.VZPPreis;
                MuenzkatalogFiltered[index].Farbe = "1";
            }

            if (args.STNPreis != 0)
            {
                MuenzkatalogFiltered[index].STNPreis = args.STNPreis;
                MuenzkatalogFiltered[index].Farbe = "1";
            }

            if (args.STHPreis != 0)
            {
                MuenzkatalogFiltered[index].STHPreis = args.STHPreis;
                MuenzkatalogFiltered[index].Farbe = "1";
            }

            if (args.PPPreis != 0)
            {
                MuenzkatalogFiltered[index].PPPreis = args.PPPreis;
                MuenzkatalogFiltered[index].Farbe = "1";
            }

            var bestand = DatabaseHelper.LiteDatabase.GetBestand(args.Guid);
            ...
            {
                case enmSelectedStyle.SammlungUndDoubletten:
                    summe += (bestand.S + bestand.DS) * MuenzkatalogFiltered[index].SPreis;
                    summe += (bestand.SP + bestand.DSP) * MuenzkatalogFiltered[index].SPPreis;
                    summe += (bestand.SS + bestand.DSS) * MuenzkatalogFiltered[index].SSPreis;
                    summe += (bestand.SSP + bestand.DSSP) * MuenzkatalogFiltered[index].SSPPreis;
                    summe += (bestand.VZ + bestand.DVZ) * MuenzkatalogFiltered[index].VZPreis;
                    summe += (bestand.VZP + bestand.DVZP) * MuenzkatalogFiltered[index].VZPPreis;
                    summe += (bestand.STN + bestand.DSTN) * MuenzkatalogFiltered[index].STNPreis;
                    MuenzkatalogFiltered[index].SummeS = summe != 0 ? string.Format("{0:#,##0.00}", summe) : string.Empty;

                    summe = 0;
                    summe += (bestand.STH + bestand.DSTH) * MuenzkatalogFiltered[index].STHPreis;
                    summe += (bestand.PP + bestand.DPP) * MuenzkatalogFiltered[index].PPPreis;
                    MuenzkatalogFiltered[index].SummePP = summe != 0 ? string.Format("{0:#,##0.00}", summe) : string.Empty;
                    break;

                case enmSelectedStyle.DoublettenOnly:
                    summe += bestand.DS * MuenzkatalogFiltered[index].SPreis;
                    summe += bestand.DSP * MuenzkatalogFiltered[index].SPPreis;
                    summe += bestand.DSS * MuenzkatalogFiltered[index].SSPreis;
                    summe += bestand.DSSP * MuenzkatalogFiltered[index].SSPPreis;
                    summe += bestand.DVZ * MuenzkatalogFiltered[index].VZPreis;
                    summe += bestand.DVZP * MuenzkatalogFiltered[index].VZPPreis;
                    summe += bestand.DSTN * MuenzkatalogFiltered[index].STNPreis;
                    MuenzkatalogFiltered[index].SummeS = summe != 0 ? string.Format("{0:#,##0.00}", summe) : string.Empty;

                    summe = 0;
                    summe += bestand.DSTH * MuenzkatalogFiltered[index].STHPreis;
                    summe += bestand.DPP * MuenzkatalogFiltered[index].PPPreis;
                    MuenzkatalogFiltered[index].SummePP = summe != 0 ? string.Format("{0:#,##0.00}", summe) : string.Empty;
                    break;

                case enmSelectedStyle.Icon:
                case enmSelectedStyle.SammlungOnly:
                    summe += bestand.S * MuenzkatalogFiltered[index].SPreis;
                    summe += bestand.SP * MuenzkatalogFiltered[index].SPPreis;
                    summe += bestand.SS * MuenzkatalogFiltered[index].SSPreis;
                    summe += bestand.SSP * MuenzkatalogFiltered[index].SSPPreis;
                    summe += bestand.VZ * MuenzkatalogFiltered[index].VZPreis;
                    summe += bestand.VZP * MuenzkatalogFiltered[index].VZPPreis;
                    summe += bestand.STN * MuenzkatalogFiltered[index].STNPreis;
                    MuenzkatalogFiltered[index].SummeS = summe != 0 ? string.Format("{0:#,##0.00}", summe) : string.Empty;

                    summe = 0;
                    summe += bestand.STH * MuenzkatalogFiltered[index].STHPreis;
                    summe += bestand.PP * MuenzkatalogFiltered[index].PPPreis;
                    MuenzkatalogFiltered[index].SummePP = summe != 0 ? string.Format("{0:#,##0.00}", summe) : string.Empty;
                    break;
            }
        }
