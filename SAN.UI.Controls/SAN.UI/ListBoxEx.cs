...
namespace Global
{
	...
	public class ListBoxEx : ListBox
	{
		...
		public object GetItemAt(int rowIndex, int columnIndex)
		{
			...
			{
				if (rowIndex < Items.Count && rowIndex >= 0)
				{
					PropertyDescriptorCollection pdc = DataManager.GetItemProperties();
					...
					else
					{
						try
						{
							result= pdc[columnIndex].GetValue(Items[rowIndex]);
						}
						catch
						{
						}
		...
		{
			base.OnDrawItem(e);
			if (!this.DesignMode && DataManager != null)
			{
		...
		public class ColumnWidthCollection
		{
			...
			protected ListBoxEx parent;
			public ColumnWidthCollection(ListBoxEx parent)
			{
